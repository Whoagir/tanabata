# ore_renderer.gd
# Визуализация руды (пульсирующие круги). Цифры запаса на гексе не показываются.
extends Node2D
...
func _create_ore_visual(ore_id: int, ore: Dictionary, pos: Vector2):
	...
	# Лейбл запаса (скрыт, цифры на руде не показываем)
	var label = Label.new()
