...
### Уже реализовано
- Смена фазы — по клику по индикатору (в режиме разработчика I) или автоматически (5 башен → SELECTION, 2 выбранных → WAVE, конец волны → BUILD)
- `P` — пауза (в game_root)
- `B` — книга рецептов
- `U` — режим редактирования энерголиний (перетаскивание линий от башни к башне)
...
│   ├── waves.json             # Создано
│   ├── wave_balance.json      # Множители HP врагов по волнам
│   ├── ability_definitions.json
│   └── mimic_weights.json     # Веса для крафта Мимика
│
