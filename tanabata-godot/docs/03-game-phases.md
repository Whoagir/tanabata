...
- В режиме разработчика (I) клик по индикатору фазы переводит в WAVE.
...
- Счётчик башен сбрасывается, очередь стеша (stash_queue) заполняется начальными 5 слотами Б/А по правилам волны.
- Можно строить следующие 5 башен; в HUD слева снизу отображается оставшаяся очередь слотов (при постановке — слот забирается, при снятии майнера/батареи — Б возвращается в очередь).
...
В режиме разработчика (клавиша I) клик по индикатору переводит на следующую фазу (вручную). В обычном режиме индикатор не кликабелен.
