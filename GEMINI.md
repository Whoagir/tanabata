...
9. Введи очень краткий и лаконичный блокнот разработки, фиксируй в нем кратко все что делаешь, назови его дневник.txt
10. ПРИ ПЕРВОМ ЗАПРОСЕ ИЗУЧАЙ ПРОЕКТ ПЕРЕД ОТВЕТОМ
11. НИКОГДА И НИ В КОЕМ СЛУЧАЕ НЕ ПОДТВЕРЖДАЙ СВОИ ЖЕ ПЛАНЫ И ДЕЙСТВИЯ, ТОЛЬКО С МОЕГО РАЗРЕШЕНИЯ
12. При двух ошибка используй вместо replace write_file для перезаписи файла целиком. Это более
...
[
  {
    "input_ids": ["TOWER_PHYSICAL_ATTACK", "TOWER_MAGICAL_ATTACK", "TOWER_PURE_ATTACK"],
...
Так же у меня есть разные типы атаки, сплит, соло, планирую добавить атаку по гексам как маяк, по области. 
...
Дальше у меня есть две основные состовляющие, методы работы с гексами (hexmap logic и hexmap render), а так же это ECS. Методы работы с гексами вынесены в отдельный пакет и нужны для многих расчётов, а так же для пререндера карты. Методы работы с гексами так же содержет пререндер карты, он идет в hex_renderer, кроме этого ничего там не надо рендерить. ЕЩЁ РАЗ В her_renderer ПОЛНОЦЕННО И ОНО ПРЕРЕНДЕРИТ КАРТУ.
